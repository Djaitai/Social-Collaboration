app.factory('BlogService', function($http, $q, $rootScope) {
	...
	return {
		...
		listBlog : function() {
			...
			return $http.get(BASE_URL + "admin_getAllBlogs").then(
					function(response) {
		...
		getBlog : function(blog_title) {
			console.log("Get Blog in Service: " + BASE_URL + "getBlog-"
					+ blog_title)
			return $http.get(BASE_URL + "getBlog-" + blog_title).then(
					function(response) {
						console.log(response.status)
		...
		getComments : function(blog_title) {
			console.log("Get Comments for " + BASE_URL + "getComments-"
					+ blog_title)
			return $http.get(BASE_URL + "getComments-" + blog_title).then(
					function(response) {
						console.log(response.status)
